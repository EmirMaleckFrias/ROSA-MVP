...
export function motivoNoAceptable(h: Pick<Hipotesis, 'afirmaciones' | 'hallazgos' | 'comprobacion'>): string | null {
  ...
  if (bloqueantes > 0) return `${bloqueantes} ${bloqueantes === 1 ? 'afirmacion bloquea' : 'afirmaciones bloquean'} la aceptacion: hay que corregirlas o quitarlas`;
  ...
  if (h.comprobacion.biomarcador.trim() === '' && h.comprobacion.cohorte.trim() === '') {
    return 'La hipotesis no dice con que biomarcador ni con que cohorte se comprobaria';
  }
  return null;
}

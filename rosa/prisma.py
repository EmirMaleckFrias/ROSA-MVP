...
def _markdown(inv: dict[str, Any], corrida: dict[str, Any], flujo: dict[str, Any], items: dict[str, Any], lsr: dict[str, Any], traice: dict[str, Any], ahora: int) -> str:
    ...
    L += ["", "## Declaración de la IA usada (PRISMA-trAIce)", "", f"- Modelos de cribado: {', '.join(traice['M2_modelos']['cribado']) or 'sin llamadas registradas'}; verificación y Killer: {', '.join(traice['M2_modelos']['verificacion_y_killer']) or 'sin llamadas registradas'}; acceso por {traice['M2_modelos']['accesoPor']}", f"- Prompts: {json.dumps(traice['M6_prompts'], ensure_ascii=False)}", f"- Umbral de inclusión automática: relevancia >= {traice['M7_umbrales']['relevanciaMinima']} de 10", f"- Revisión humana: {traice['M8_revision_humana']['decisionesDePersonas']} decisiones de personas sobre {traice['M8_revision_humana']['decisionesDelKiller']} del Killer; {int(traice['M8_revision_humana']['descartesAuditadosPorOtroModelo'] * 100)} % de los descartes auditados por otro modelo", f"- Acuerdo con personas: kappa global {traice['M9_acuerdo_con_personas']['kappaGlobal']} sobre {traice['M9_acuerdo_con_personas']['conjuntoDorado']} etiquetas del conjunto dorado", f"- {traice['R1_flujo_distingue_ia_y_humano']}"]
    return "\n".join(L)

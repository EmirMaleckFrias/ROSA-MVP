...
F = V.Fragmento
FRAGS = [
    F("f1", "Cohorte clinica, 2025", "pág. 7", "El cociente alcanzo una precision de 0,91 (IC 95 % 0,86-0,95) en la cohorte FLENI, con el ensayo NCT01234567 como referencia.", "Resultados"),
    F("f2", "Cummings et al., 2026", "resumen", "En 2026 hay 138 farmacos en 182 ensayos; lecanemab y donanemab estan aprobados.", ""),
]
...
def test_cita_que_no_resuelve_y_sin_cita():
    assert c("Hay 138 farmacos.", "[Cummings et al., 2026, pág. 3]").veredicto == "cita_no_resuelve"
    assert c("Hay 138 farmacos.", "").veredicto == "sin_cita"
    assert c("Hay 138 farmacos.", "[Cummings et al., 2026, resumen]", "texto que no esta").veredicto == "cita_no_resuelve"


def test_ausencia_refutada_y_honesta():
    assert c("No encuentro informacion sobre lecanemab en los documentos.", "").veredicto == "ausencia_refutada"
    assert c("No encuentro informacion sobre aducanumab en los documentos.", "").veredicto == "sostenida"
    assert c("No pude comprobar lo de lecanemab.", "").veredicto == "sostenida"
    assert c("No hay datos sobre GEN 1 pero la precision fue 0,91.", "").veredicto == "sin_cita"
